      ...
      steps {
        node(label: 'docker') {
          withCredentials([string(credentialsId: 'eea-jenkins-token', variable: 'GITHUB_TOKEN')]) {
           sh ''' ls -la ./; rm -rf ./$GIT_NAME'''
           sh ''' git clone https://$GIT_USER:$GITHUB_TOKEN@github.com/eea/${GIT_NAME}.git ./$GIT_NAME$GIT_COMMIT; cd ./$GIT_NAME$GIT_COMMIT; git reset --hard origin/master'''
           ...
           sh ''' cd ..; rm -rf ./$GIT_NAME$GIT_COMMIT''' 
          }
        }
      }
    }
